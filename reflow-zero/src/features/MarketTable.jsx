...
export const MarketTable = ({ data, onRowClick }) => {
    ...
    const rowVirtualizer = useVirtualizer({
        ...
        estimateSize: () => 56, // Row height chart ke hisab se
        overscan: 5,
    });
                ...
                <div style={{ height: `${rowVirtualizer.getTotalSize()}px`, width: '100%', position: 'relative' }}>
                    {rowVirtualizer.getVirtualItems().map((virtualRow) => {
                        ...
                        return (
                            <div
                                key={coin.symbol}
                                onClick={() => onRowClick(coin.symbol)} // ADDED: Click handler for modal
                                className="absolute top-0 left-0 w-full grid grid-cols-4 md:grid-cols-6 lg:grid-cols-7 gap-4 px-4 border-b border-gray-700 hover:bg-gray-750 transition-colors text-sm items-center cursor-pointer" // ADDED: cursor-pointer
                                style={{
